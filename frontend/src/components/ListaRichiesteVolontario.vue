    ...
    <div class="card card-container">
      ...
      >
        ...
        <template v-slot:item.accetta="{ item }">
          <v-icon icon="fas fa-hand-holding-heart"  @click="accetta(richiesta)"></v-icon>
        </template>
      </v-data-table>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async conferma() {
      try {
        const res = await ServizioVolontario.accettaRichiesta(this.richieste[this.richiestaAccettata]._id);
        // TODO mostrare un messaggio di conferma
      } catch (err) {
